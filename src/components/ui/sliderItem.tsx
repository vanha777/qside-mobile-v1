...
import { scan, textRecord, uriRecord, write, WriteOptions, ScanKind } from "@tauri-apps/plugin-nfc";
...
const SliderItem: React.FC<SliderItemProps> = ({ socialUser, fetchSocial }) => {
    ...
    const activateReadNfc = async () => {
        ...
        setNfcResponse("trying to read Nfc");
        try {
            const read = await scan({ type: "tag" }, { message: "Reading", successMessage: "Detected Tag" });
            console.log("activate nfc ", read.records);
            setNfcResponse(read.records);
        } catch (error) {
    ...
    const activateWritedNfc = async () => {
        ...
        setNfcResponse("trying to write Nfc");
        // const read = await scan({ type: "tag", keepSessionAlive: true });
        try {
            const writeInfo = await write([uriRecord("https://www.tiktok.com/@eazyhomeiot")], { kind: { type: "ndef" }, message: "reading tag", successMessage: "Detected Tag", successfulReadMessage: "Finished write tag", });
